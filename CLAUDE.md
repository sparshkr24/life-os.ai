...
| 15 | **done** | **Self-learning loop closed.** `reinforceMemory` now bumps `confidence` (+0.05, cap 0.99) and `contradictMemory` drops it (−0.10, floor 0.05). New `client/src/memory/maintenance.ts` runs a deterministic SQL sweep at the end of every `runMemoryPass` (after the LLM tool loop succeeds): soft-archives failed predictions with `was_correct=0 AND reinforcement=0`, consistently-disproven memories (`contradiction≥3 AND contradiction≥2×reinforcement`), and `confidence<0.10 AND reinforcement=0`. The memory pass prompt now surfaces “shaky memories” (contradictions or low-confidence) so the LLM can revise/reinforce/contradict against today's evidence; bookkeeping archival is no longer the LLM's job. |
| 16 | **done** | **Pattern abstraction + merging.** Daily memory pass already invokes `consolidate_memories` (LLM tool) when it sees ≥3 specific memories sharing a pattern — creates an abs.tract `pattern`-typed parent with `parent_id`/`child_ids` linkage. Stage-15 maintenance sweep now also soft-archives consolidation children once the parent has been alive ≥14 days (grace period to confirm the abstract is stable). No separate weekly pass — consolidation rides the same nightly memory tool loop the user opted into. |
...
| **v7** | **done** | **Proactive AI + interactive nudges.** Schema v7 (`proactive_questions`, EventKinds `ai_question`/`ai_question_response`, `LlmPurpose='proactive_question'`). New native bridge method `LifeOsBridge.getCurrentLocation()` (Kotlin FusedLocationProvider). Settings → Places UI for managing geofenced locations (add/edit-radius/delete, syncs OS geofences). Aggregator hook `maybeRunProactiveQuestion()` runs after predictive insights — three deterministic detectors (long_dwell_unknown / weekend_late_night / no_phone_usage) with hard caps (≥120 min throttle, ≤3/day, no pending, 24 h same-trigger cooldown), LLM drafts a single Q via `runChatTask('proactive_question', …, jsonMode)`. Interactive notifications via three `setNotificationCategoryAsync` categories (yes/no, place_name, free_text) — Yes/No resolves in tray, Other/Reply opens app. Pending question card on Today self-fetches the latest `status='pending'` row; place_name answers from the in-app card auto-save via `addPlace`. New chat tools: `add_geofence_place`, `mark_pattern_memory`, `ask_user_question`. Engine: [client/src/brain/proactive.ts](client/src/brain/proactive.ts), notification adapter: [client/src/rules/proactiveNotify.ts](client/src/rules/proactiveNotify.ts). |
...
13. **Schema is at v7 (additive).** v3 added `daily_rollup.productivity_score`, `nudges_log.{next_day_score,baseline_score,score_delta,user_helpful}`, EventKinds `inferred_activity`/`user_clarification`. v4 added the `memories` table. v6 added `rules.{source,predicted_impact_score,based_on_memory_ids,disabled_reason,last_refined_ts}`. **v7 adds the `proactive_questions` table + EventKinds `ai_question`/`ai_question_response` + `LlmPurpose='proactive_question'` + ProactiveTriggerKind union.** `user_helpful` is the user's manual thumbs feedback (1=▲, -1=▼, NULL=unrated), INDEPENDENT of the LLM-computed `score_delta`. Migrations use `addColumnIfMissing` (PRAGMA-guarded `ALTER TABLE`) + `CREATE TABLE IF NOT EXISTS`. Never DROP or RENAME.
