...
| `repos/places.ts` | `listPlaces`, `addPlace` (accepts `kind`/`confidence`/`category`), `updatePlaceRadius`, `setPlaceKind`, `setPlaceLastVisit`, `deletePlace`, `syncGeofences()`. Every mutation re-pushes the full `places` table to the bridge — including `kind='ignored'` places (they stay registered so we can suppress dwell when the user enters them). `PLACES_DEFAULT_RADIUS_M = 25`; auto-detected places get 50m. |
| `geocode/geocoder.ts` | `reverseGeocode(lat,lng)` → Nominatim wrapper with quantized 24h cache (`geocode_cache`). 1 req/sec throttle. Sends a per-install UUID in `User-Agent` (read/written via `getInstallId()` against `schema_meta.install_id`) so the 10k/day quota is per-user. Maps OSM `class`/`type` to a coarse vocabulary (cafe, restaurant, shop_alcohol, …). Confidence ∈ {0.3, 0.55, 0.85}. |
| `geocode/geocodeWorker.ts` | `processPendingPlaceVisits()` — drains up to 5 `place_visit` rows where `payload.status='pending_geocode'` per aggregator tick. High confidence (≥0.8) auto-creates a `places` row with `kind='auto'` (continuity: future visits trigger geofence, no Nominatim). Low confidence (<0.6) inserts a `proactive_questions` row with `expected_kind='place_name'` and fires a notification (existing answer flow → `addPlace` creates the manual place). Also exports `closeOrphanedOpenPlaceVisits` used by the cleanup pipeline. |
