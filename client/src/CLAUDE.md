...
| `aggregator/index.ts` | `runAggregatorTick()` — the public entry point. Per tick: `purgeShortAppFg` → `classifySilences(today,yest)` → `rebuildDailyRollup(today,yest)` → `computeProductivityScore(today,yest)` → once-per-day `foldMonth(prevMonth)` (gated via `schema_meta.last_monthly_fold_date`) → `runRulesOnceFromBackground()`. Records `last_aggregator_ts`. Returns `TickReport`. |
...
| `screens/Today.tsx` | `TodayScreen` — counts, collector-service health, LLM spend, profile summary, aggregator status (registered? last-tick ts) + `Run aggregator now`, rule-engine status (last-tick ts) + `Run rules now` debug button. |
